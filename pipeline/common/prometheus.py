...
Used by services that previously hand-rolled identical formatting inside
`metrics_lines(service_name)`. The builder only renders; callers retain
their counter bookkeeping and locking. NOT thread-safe — callers hold
their own locks (matches the existing pattern).
