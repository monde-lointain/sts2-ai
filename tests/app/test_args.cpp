// Tests for src/app/Args.{h,cpp} — main.cpp helpers hoisted per §2.1.
...
// T-MAIN-045 — EG — Leading whitespace " 5" → false (caller is expected to trim).
...
TEST(AppParseArgs, T_MAIN_050_NoArgsOk) {
    ...
    EXPECT_TRUE(parse_args(static_cast<int>(argv.size()), argv.data(),
                           seed, seed_provided, err));
...
TEST(AppParseArgs, T_MAIN_060_MissingValueErrors) {
    ...
    EXPECT_FALSE(parse_args(static_cast<int>(argv.size()), argv.data(),
                            seed, seed_provided, err));
