// Tests for src/game/Combat.{h,cpp}.
...
// are toolchain-locked via tests/seeds/expected_values.h (kSilentDeckShuffled_C0FFEE).
// Most assertions here check state (counts, HP, pile sizes, power vectors)
// rather than specific card orderings, so the seed pin only matters where
// noted. Cultist HP rolls under that seed are observed via the public API
// (e.g. enemies()[0].vitals.hp) without re-pinning concrete numbers.
//
...
std::vector<Card> MakeStrikeDefendDeck7() {
    std::vector<Card> d;
    d.push_back(cards::make_strike());
    d.push_back(cards::make_defend());
    d.push_back(cards::make_strike());
    d.push_back(cards::make_defend());
    d.push_back(cards::make_strike());
    d.push_back(cards::make_defend());
...
int FindHandIndex(const Combat& c, CardId id) {
    ...
    for (std::size_t i = 0; i < h.size(); ++i) {
        if (h[i].id == id) return static_cast<int>(i);
    }
    return -1;
}
...
TEST(CombatDelegators, T_CMB_035_ApplyPowerToEnemyAccumulates) {
    ...
    c.apply_power_to_enemy(0, PowerKind::Weak, 1);
    ASSERT_EQ(c.enemies()[0].vitals.powers.size(), 1u);
    EXPECT_EQ(c.enemies()[0].vitals.powers[0].kind, PowerKind::Weak);
    EXPECT_EQ(c.enemies()[0].vitals.powers[0].amount, 1);

    c.apply_power_to_enemy(0, PowerKind::Weak, 2);
    ASSERT_EQ(c.enemies()[0].vitals.powers.size(), 1u);
...
TEST(CombatDelegators, T_CMB_040_ApplyPowerToEnemySelf) {
    ...
    c.apply_power_to_enemy_self(e, PowerKind::Ritual, 2);

    ASSERT_EQ(e.vitals.powers.size(), 1u);
    EXPECT_EQ(e.vitals.powers[0].kind, PowerKind::Ritual);
...
// T-CMB-050 — BP, DF — deal_damage_to_enemy(0, 6) with no powers reduces hp by 6.
...
// T-CMB-055 — EG — Lethal damage on one of two enemies does NOT trip combat_over.
TEST(CombatDealDamage, T_CMB_055_LethalWhileOtherAlive) {
    Combat c{kCombatTestSeed};
    Enemy e0{}; e0.vitals = Vitals{1, 1, 0, {}};
    Enemy e1{}; e1.vitals = Vitals{40, 40, 0, {}};
...
// T-CMB-060 — EG — Lethal to the LAST enemy trips combat_over via all_enemies_dead.
...
TEST(CombatDealDamage, T_CMB_065_EnemyAttackUsesSourcePowers) {
    Combat c{kCombatTestSeed};
    Enemy source{};
    source.vitals = Vitals{40, 40, 0, { MakePower(PowerKind::Strength, 2) }};
...
TEST(CombatCanPlay, T_CMB_085_LastValidIdxUnaffordable) {
    ...
    std::vector<Card> deck;
    for (int i = 0; i < 6; ++i) deck.push_back(cards::make_strike());
    c.start(std::move(deck));
...
TEST(CombatCanPlay, T_CMB_095_ZeroCostZeroEnergy) {
    ...
    while (strikes_played < 3) {
        int idx = FindHandIndex(c, CardId::Strike);
        ASSERT_NE(idx, -1);
    ...
    int neut_idx = FindHandIndex(c, CardId::Neutralize);
...
// Energy=3 by default after start; to be "unplayable", we drain it then attempt.
TEST(CombatPlayCard, T_CMB_105_UnplayableReturnsFalse) {
    ...
    EXPECT_EQ(c.player().hand[0].id, CardId::Strike);
    EXPECT_EQ(c.player().energy, 0);
...
TEST(CombatPlayCard, T_CMB_115_StrikePlaysAndDealsDamage) {
    ...
    EXPECT_TRUE(c.player().hand.empty());
    ASSERT_EQ(c.player().discard_pile.size(), 1u);
    EXPECT_EQ(c.player().discard_pile[0].id, CardId::Strike);
...
TEST(CombatPlayCard, T_CMB_120_SurvivorBlocksAndDiscards) {
    ...
    c.set_pick_discard_callback([](const Combat& cc) {
        for (std::size_t i = 0; i < cc.player().hand.size(); ++i) {
            if (cc.player().hand[i].id == CardId::Strike) return static_cast<int>(i);
        }
        return 0;
    });
    std::vector<Card> deck;
    deck.push_back(cards::make_survivor());
    deck.push_back(cards::make_strike());
    deck.push_back(cards::make_defend());
    c.start(std::move(deck));
    ASSERT_EQ(c.player().hand.size(), 3u);

    int survivor_idx = FindHandIndex(c, CardId::Survivor);
    ...
    EXPECT_TRUE(ok);
    ASSERT_EQ(c.player().hand.size(), 1u);
    EXPECT_EQ(c.player().hand[0].id, CardId::Defend);
    ASSERT_EQ(c.player().discard_pile.size(), 2u);
    EXPECT_EQ(c.player().discard_pile[0].id, CardId::Strike);
    EXPECT_EQ(c.player().discard_pile[1].id, CardId::Survivor);
...
TEST(CombatDraw, T_CMB_145_ReshuffleMidLoop) {
    Combat c{kCombatTestSeed};
    std::vector<Card> deck;
    for (int i = 0; i < 8; ++i) deck.push_back(cards::make_defend());
    c.start(std::move(deck));
    ASSERT_EQ(c.player().hand.size(), 7u);
...
TEST(CombatReshuffle, T_CMB_160_NonEmptyDiscardDrainedAndShuffled) {
    ...
    for (int i = 0; i < 3; ++i) {
        int idx = FindHandIndex(c, CardId::Defend);
        ASSERT_NE(idx, -1);
    ...
    for (const Card& cc : c.player().draw_pile)    expected_ids.push_back(cc.id);
    for (const Card& cc : c.player().discard_pile) expected_ids.push_back(cc.id);
    ...
    for (const Card& cc : c.player().draw_pile) got_ids.push_back(cc.id);
...
TEST(CombatStartPlayerTurn, T_CMB_180_Round2ResetsBlockDrawsFive) {
    ...
    ASSERT_EQ(c.enemies().size(), 2u);
    EXPECT_EQ(c.enemies()[0].current_move, MoveId::DarkStrike);
    EXPECT_EQ(c.enemies()[1].current_move, MoveId::DarkStrike);
}
...
TEST(CombatStartPlayerTurn, T_CMB_185_DeadEnemyMoveNotRolled) {
    ...
    EXPECT_EQ(c.enemies()[0].current_move, MoveId::Incantation);
    EXPECT_TRUE(c.enemies()[0].performed_first_move);
...
TEST(CombatEnemyPhase, T_CMB_195_TwoAliveIncantationFullSweep) {
    ...
    ExpectPowersEq(c.enemies()[0].vitals.powers,
                   { MakePower(PowerKind::Ritual, 2, /*just_applied=*/false) });
    ExpectPowersEq(c.enemies()[1].vitals.powers,
                   { MakePower(PowerKind::Ritual, 5, /*just_applied=*/false) });
...
TEST(CombatEnemyPhase, T_CMB_200_Round2DarkStrikeRitualToStrength) {
    ...
    c.end_turn();  // run R2 end_player_turn → enemy_phase → (round inc + R3 start)
    ...
    ExpectPowersEq(c.enemies()[0].vitals.powers,
                   { MakePower(PowerKind::Ritual,   2, /*just_applied=*/false),
                     MakePower(PowerKind::Strength, 2) });
    ExpectPowersEq(c.enemies()[1].vitals.powers,
                   { MakePower(PowerKind::Ritual,   5, /*just_applied=*/false),
                     MakePower(PowerKind::Strength, 5) });
}
...
TEST(CombatEndTurn, T_CMB_220_PlayerDeathDuringEnemyPhaseEarlyReturn) {
    ...
    EXPECT_EQ(c.round(), 2);
    // R3 start_player_turn was NOT called → hand stays empty (post end_player_turn).
    EXPECT_TRUE(c.player().hand.empty());
}

// T-CMB-225 — BP — Normal round transition: full path through
// end_player_turn → enemy_phase → round_++ → start_player_turn → check_win_or_lose.
...
TEST(CombatAllEnemiesDead, T_CMB_250_AliveInMiddleFalse) {
    Combat c{kCombatTestSeed};
    Enemy a{}; a.vitals = Vitals{1, 1, 0, {}};
    Enemy b{}; b.vitals = Vitals{40, 40, 0, {}};
    Enemy d{}; d.vitals = Vitals{1, 1, 0, {}};
...
// T-CMB-255 — BP — Player dead → combat_over true (left of `||` short-circuits).
// Use a no-enemy combat so all_enemies_dead would be FALSE (empty vector);
// only is_player_dead can flip combat_over here.
...
// T-CMB-290 — BP, DF — Callback returns valid index → moves hand[idx] to discard.
