...
TEST(combat_round_one_does_not_clear_player_block) {
    Combat c{1};
    c.start(cards::make_silent_starter_deck());
    CombatTestAccess{c}.player().vitals.block = 5;
...
TEST(combat_round_two_clears_player_block) {
    ...
    c.add_enemy(make_dummy_enemy(100));
    c.start(cards::make_silent_starter_deck());
    CombatTestAccess{c}.player().vitals.block = 12;
    c.end_turn();
...
TEST(combat_can_play_returns_false_when_no_energy) {
    Combat c{1};
    CombatTestAccess{c}.player().energy = 0;
    CombatTestAccess{c}.player().hand.push_back(cards::make_strike());
    CHECK(!c.can_play(0));
}
...
TEST(combat_play_strike_kills_low_hp_enemy) {
    Combat c{1};
    CombatTestAccess{c}.player().energy = 3;
    c.add_enemy(make_dummy_enemy(5));
    CombatTestAccess{c}.player().hand.push_back(cards::make_strike());
    c.play_card(0, 0);
...
TEST(combat_killing_last_enemy_marks_combat_over) {
    Combat c{1};
    c.add_enemy(make_dummy_enemy(2));
    CombatTestAccess{c}.player().energy = 3;
    CombatTestAccess{c}.player().hand.push_back(cards::make_strike());
    c.play_card(0, 0);
    CHECK(c.enemies()[0].vitals.hp == 0);
