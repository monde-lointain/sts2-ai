...
TEST(InputTrim, T_INP_105_Empty) { EXPECT_EQ(trim(""), ""); }

// T-INP-110 — BP, BV — All whitespace → empty.
TEST(InputTrim, T_INP_110_AllWhitespace) { EXPECT_EQ(trim("    "), ""); }
...
TEST(InputTrim, T_INP_125_BothEnds) { EXPECT_EQ(trim("   abc   "), "abc"); }

// T-INP-130 — BP — No whitespace → unchanged.
TEST(InputTrim, T_INP_130_NoWhitespace) { EXPECT_EQ(trim("abc"), "abc"); }
