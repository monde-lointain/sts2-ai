// Tests for hoisted Render anon-namespace helpers in src/render/render_internal.h.
// Spec: docs/test-plan/02-test-specifications.md §11.2 (T-RND-065..150).
//
...
TEST(RenderInternalRepeat, T_RND_070_CountThreeRepeats) {
    ...
    EXPECT_EQ(s, std::string(glyphs::kFullBlock) + glyphs::kFullBlock + glyphs::kFullBlock);
}
...
TEST(RenderInternalFormatPowers, T_RND_110_TwoWithSeparator) {
    const std::string s = format_powers({
        MakePower(Weak, 2), MakePower(Strength, 3) });
    const std::string expected =
        std::string(ansi::kReset) + "Weak 2"   + ansi::kReset + ", " +
        std::string(ansi::kReset) + "Str 3"    + ansi::kReset;
    EXPECT_EQ(s, expected);
}
...
TEST(RenderInternalMaxEnemyName, T_RND_140_AllAliveMaxLen) {
    Enemy a{}; a.name = "ab";    a.vitals = Vitals{10, 10, 0, {}};
    Enemy b{}; b.name = "abcde"; b.vitals = Vitals{10, 10, 0, {}};
    Enemy c{}; c.name = "abcd";  c.vitals = Vitals{10, 10, 0, {}};
...
TEST(RenderInternalMaxEnemyName, T_RND_145_DeadExcluded) {
    Enemy a{}; a.name = "longer"; a.vitals = Vitals{0, 10, 0, {}};
    Enemy b{}; b.name = "x";      b.vitals = Vitals{10, 10, 0, {}};
