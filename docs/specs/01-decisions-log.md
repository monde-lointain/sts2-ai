...
**R12 (DISCHARGED 2026-05-21 on wave-45 close).** Mid-combat behavioral drift undetected by `SyncStatePinGate` + `DllSignatureGate` + `probe-upstream-initial-state` was the residual gap surfaced by R11's permanent escalation. ADR-035 (Accepted Amended 2026-05-21) ratified the mitigation: mid-combat reflective per-turn probe (Q1-A1) + Roslyn analyzer for inline-upstream-comment enforcement (Q1-A3) + Q4 DSL extractor + coherence invariants (Q4-B1, Q4-B2) + red-team validation evidence (Q1-R1). Wave-45 closed at HEAD `aa3aa08` with 4/5 red-team injections firing PASS + 1 QUALIFIED (LouseProgenitor action-sequence coverage gap; wave-46/Q1-A2 remediates). Discharge gated on R.1 evidence artifact landing (`engine/headless/docs/specs/drift-gate-hardening-redteam-evidence.md`) — landed at commit `aa3aa08`. Phase-1.5 substrate fill-in for net-new encounters UNPAUSES on wave-45 close.
