...
- **`[ASPIRATION → FALSIFIED post-Wave-6.5; see ADR-034]`** Run as out-of-tree mod via `Core/Modding` where possible; minimize patches to the upstream tree to keep rebases cheap (per ADR-002 positives). This aspiration was blocked at Wave-6.5 by the `Core/Logging/Logger..cctor` → `Godot.OS.GetCmdlineArgs()` SIGSEGV wall; Q1 transitioned to a parallel substrate in `Sts2Headless.Domain.*` that does not use `Core/Modding` at all. T3 Ledger has **0 entries** to date — not because the T1/T2 discipline was exercised against live `Core/Modding`, but because the parallel substrate never needed upstream tree patches. See ADR-034 for the full substrate-vs-port distinction and Wave-6.5 retrofit history.
...
- **`[SHIPPED]`** **Sync — IPC (hot path, ADR-005):** hook protocol over shared-memory ring buffer + two semaphores. Per-decision target <500µs (measured p99 = 14.24 µs, ~35× margin). Caller is Q8. **Q12 and Q11 consumption [PHASE-2]** (Q12 evaluation harness and Q11 curriculum-generator integration follows when those quanta boot).
    - **`[SHIPPED]`** **Power-side OnDeath substrate (wave-26 close, ADR-030):** `HookType.AfterDeath` and `HookType.ShouldStopCombatFromEnding` are now fired by `CombatEngine` at the creature-death and `CheckCombatEnd` sites respectively; `PowerModel` instances subscribe via the per-attachment `OnApplied → SubscribeHooks → tracked-handles → OnRemoved` lifecycle, and mid-combat enemy spawns are available via `ICombatContext.AddEnemies(...)`. `ShouldStopCombatFromEnding` uses the `HookContext` mutable-flag pattern (boolean veto via side channel — `HookRegistry.Fire` remains `void`-returning). No `contracts/schemas/` change; wire format unchanged. See ADR-030 for the determinism contract and the forward-look on additive consumers.
