...
- **Counterfactual evaluator.** For map decisions, run the not-taken path under simulation and compare. Surfaces map-policy deficiencies.
...
- **Phase 1.** Combat-only: ≥95% A0 normal-encounter win rate; ≥90% expectimax agreement; latency <100ms at 64-sim budget. Pinned regression battery covers the existing 252-test scope plus oracle agreement on tractable states.
