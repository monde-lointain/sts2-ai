...
- **[SHIPPED]** **Engine→CompactState adapter (ADR-011).** Consume Q1's versioned binary state and derive `CompactState` for verifier use. (`engine/cpp/src/oracle/adapter/` per Q2-ADR-001; Phase-1A scope is CULTISTS_NORMAL only per Q2-ADR-002 — non-cultist signatures reject with `UnsupportedEncounter` diagnostic.)
