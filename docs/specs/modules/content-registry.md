...
- **[SHIPPED]** **Token IDs.** Assign and preserve a stable integer ID for every card (per upgrade variant), relic, enemy archetype, buff/debuff, potion, event ID, and special token (`[CLS]`, `[CHAR_*]`, `[ACT_*]`, `[MASK]`, etc.). Phase-1 baseline (current 2026-05-17): canonical Q4 registry (`contracts/registry/phase1-silent.json`) holds 98 cards / 59 relics / 45 powers; monsters / potions / encounters in the canonical registry are deferred pending bridge waves per ADR-027. Q1 Phase-1 test fixture (`engine/headless/test/fixtures/q4-manifest-phase1.json`) holds 98 cards / 59 relics / 45 powers / 33 monsters / 21 potions / 0 encounters. **Caps are floors, not ceilings** — they track upstream content per ADR-027 (Phase-1 Fixture Growth Policy). Each upstream sync (per ADR-026) may grow these counts; bridge waves add canonical-registry + Q1-fixture entries in the same commit. Duplicate-ID rejection enforced by `tools/content/validate_registry.py`.
...
- **[SHIPPED]** **Token-coherence regression battery.** A Q4-specific test suite that checks: no duplicate IDs, all referenced tokens exist, deprecation log monotonic, card DSL parses. (`make content-test` → `tools/tests/content/test_registry.py`, wired into `make phase0-gate`; 12 invariant test cases pass on `phase1-silent.json` (wave-48/Q4-B1 converted 4 ex-skipped fan-in-transition clauses to hard `minor==1` preconditions; gate path validates the committed registry rather than regenerating, closing R14 #2).) **[ASPIRATION (pre-implementation)]** Embedding-init code that synthesizes embeddings for every card from its DSL alone — no such embedding-init exists today; depends on Phase-2 semantic DSL.
