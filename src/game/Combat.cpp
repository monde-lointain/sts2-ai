...
Combat::Combat(uint64_t seed)
    : rng_(seed),
      on_pick_discard_([](const Combat&) { return 0; }) {}
...
bool Combat::can_play(int hand_idx) const {
    if (hand_idx < 0 || static_cast<size_t>(hand_idx) >= player_.hand.size()) return false;
...
void Combat::gain_player_block(int amt) {
    player_.vitals.block += amt;
}
