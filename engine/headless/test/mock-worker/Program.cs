...
for (int i = 0; i < args.Length; i++)
{
    string a = args[i];
    string Next() =>
        ++i < args.Length ? args[i] : throw new ArgumentException($"missing value for {a}");
    switch (a)
    {
        case "--base-path":
            basePath = Next();
            break;
        case "--ring-capacity":
            ringCapacity = int.Parse(Next(), CultureInfo.InvariantCulture);
            break;
        case "--schema":
            schema = ushort.Parse(Next(), CultureInfo.InvariantCulture);
            break;
        case "--content-hash":
            contentHashHex = Next();
            break;
        ...
        case "-h":
            Console.WriteLine(
                "mock-worker --base-path <p> --ring-capacity <n> --schema <u16> --content-hash <hex64> [--build-id <s>] [--script echo|always-end-turn]"
            );
            return 0;
        default:
...
if (contentHashHex.Length != HookProtocolManifest.ContentHashSize * 2)
{
    Console.Error.WriteLine(
        $"--content-hash must be {HookProtocolManifest.ContentHashSize * 2} hex chars"
    );
    return 2;
}
...
using var inboundShm = SharedMemorySegment.OpenExisting(
    HookProtocolAdapter.OutboundShmPathFor(basePath),
    total
); // Q1 -> Q8 (we READ)
using var outboundShm = SharedMemorySegment.OpenExisting(
    HookProtocolAdapter.InboundShmPathFor(basePath),
    total
); // Q8 -> Q1 (we WRITE)
...
Console.Error.WriteLine(
    $"mock-worker attached: base-path={basePath} ring={ringCapacity} schema={schema}"
);
...
while (true)
{
    ...
    while (true)
    {
        if (inboundRing.AvailableToRead < MessageHeader.HeaderSize)
            break;
        if (!inboundRing.TryPeek(headerScratch))
            break;
        var hdr = MessageHeader.Decode(headerScratch);
        int wire = hdr.WireSize;
        if (inboundRing.AvailableToRead < wire)
            break;
