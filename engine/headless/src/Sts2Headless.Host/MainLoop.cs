...
public static class MainLoop
{
        ...
        IProbeStream? probe = null)
    {
        ...
        logger.Log("combat_start", new Dictionary<string, object?>
        {
            ["turn"] = ctx.State.TurnCounter,
            ["player_hp"] = ctx.State.Player.CurrentHp,
            ["enemies"] = ctx.State.Enemies.Select(e => new { id = e.Id, name = e.Name, hp = e.CurrentHp }).ToArray(),
        });
        probe?.Emit("combat_start", ctx.State);
        // Turn 1 has already been opened by CombatEngine.StartCombat.
        logger.Log("turn_start", new Dictionary<string, object?>
        {
            ["turn"] = ctx.State.TurnCounter,
        ...
        while (!ctx.State.IsCombatOver)
        {
            ...
            {
                logger.Log("shutdown", new Dictionary<string, object?>
                {
            ...
            if (legal.IsEmpty)
            {
                ...
                // combat-end.
                logger.Log("legal_actions_empty", new Dictionary<string, object?>
                {
            ...
            if (next is null)
            {
                logger.Log("script_exhausted", new Dictionary<string, object?>
                {
                    ["turn"] = ctx.State.TurnCounter,
                    ["legal_count"] = legal.Length,
                });
                return new RunResult(LoopOutcome.ScriptExhausted, turnsPlayed, actionsApplied, ctx.State);
            }
                ...
                case PlayerAction.PlayCard pc:
                    {
                        ...
                        CardInstance? inst = ctx.State.HandPile.Cards
                        ...
                        logger.Log("card_played", new Dictionary<string, object?>
                        {
                            ["turn"] = ctx.State.TurnCounter,
                            ["card"] = modelId,
                ...
                case PlayerAction.EndTurn:
                    {
                        ...
                        logger.Log("enemy_action", new Dictionary<string, object?>
                        {
                            ["turn"] = ctx.State.TurnCounter,
                            ["player_hp"] = ctx.State.Player.CurrentHp,
        ...
        LoopOutcome outcome = ctx.State.PlayerWon
            ? LoopOutcome.Victory
            : ctx.State.PlayerLost ? LoopOutcome.Defeat : LoopOutcome.ScriptExhausted;

        logger.Log("combat_end", new Dictionary<string, object?>
        {
