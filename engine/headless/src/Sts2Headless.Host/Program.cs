...
public static class Program
{
    ...
    public static int Run(string[] args, TextWriter stdout, TextWriter stderr, bool attachProcessSignals)
    {
        ...
        // --- Final state output -------------------------------------------
        string finalHash = ComputeFinalStateSha(result.FinalState, bundle);
        if (cli.OutPath is not null)
        {
            try
            {
                byte[] blob = BuildStateBlob(result.FinalState, bundle);
    ...
    private static byte[] BuildStateBlob(CombatState state, CompositionRoot.CompositionRootBundle bundle)
    {
    ...
    private static string ComputeFinalStateSha(CombatState state, CompositionRoot.CompositionRootBundle bundle)
    {
        byte[] blob = BuildStateBlob(state, bundle);
