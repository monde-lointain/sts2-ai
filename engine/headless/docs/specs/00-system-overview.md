...
3. **Patch Adaptability via Parallel-Substrate Behavior-Mirror.** STS2 is in active content-patch development. Q1's original mod-shaped boundary design ("engine strip" replacing Godot surfaces with deterministic stubs, structured as out-of-tree mod via `Core/Modding`, per pipeline ADR-002) was empirically blocked at Wave-6.5 by a P/Invoke cctor wall in `Core/Logging/Logger..cctor → Godot.OS.GetCmdlineArgs()`. Per pipeline ADR-034, Q1 is now a behavior-mirroring parallel C# substrate in `Sts2Headless.Domain.*` that treats upstream as behavioral spec, not runtime dependency. Per-component schema versioning (state, hook protocol, replay format) preserves the IPC contract across content patches; mid-combat behavioral drift detected via the upstream-sync pipeline (ADR-026) + per-encounter probes (drift-gate hardening per Phase-1.5 prerequisite). Failure mode is engineering churn per patch (1-week per-encounter port ↔ multi-week full re-port).
