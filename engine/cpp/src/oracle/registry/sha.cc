...
#error "STS2_CONTRACTS_ROOT must be defined by the build (absolute path of contracts/)"
#endif
...
std::string compute_registry_sha256(const std::filesystem::path& path) {
  ...
  if (!in) {
    throw std::runtime_error(
        "compute_registry_sha256: cannot open file: " + path.string());
  }
  const std::streamsize size = in.tellg();
  if (size < 0) {
    throw std::runtime_error(
        "compute_registry_sha256: tellg failed on file: " + path.string());
  }
  ...
  if (size > 0) {
    ...
    if (!in) {
      throw std::runtime_error(
          "compute_registry_sha256: read error on file: " + path.string());
    }
  }
