...
// Clean-room SHA-256 (FIPS 180-4). Registry-lib-internal helper; not exposed
// in any public header.
//
// NOTE: duplicates `oracle::adapter::detail::sha256`; consolidate when a 3rd
// lib needs it. Per S2 Stream A: adapter sources are off-limits; duplication
// is the cleanest R8 partition for Phase-1A. Bit-identical to the adapter
// implementation by construction (verbatim copy modulo namespace).
//
// Single-call API only:
//   std::array<uint8_t, 32> h = sha256(bytes);

namespace sts2::oracle::registry::detail {
