...
TEST(CultistsProjection, Fixture1_ProducesSaneCompactState) {
  ...
  EXPECT_FALSE(s.enemies[0].performed_first_move);
  EXPECT_FALSE(s.enemies[1].performed_first_move);
  ...
  // piles.
  EXPECT_LE(s.hand[CardId::kNeutralize] + s.draw[CardId::kNeutralize] +
                s.discard[CardId::kNeutralize],
            1);
  EXPECT_LE(s.hand[CardId::kSurvivor] + s.draw[CardId::kSurvivor] +
                s.discard[CardId::kSurvivor],
            1);
}
...
TEST(CultistsProjection, Fixture1_EnemyParamsMatchCppPrototype) {
  ...
  for (std::size_t i = 0; i < 2; ++i) {
    ...
    const auto& projected = s.enemies[i];
    if (wire.name == "CalcifiedCultist") {
      EXPECT_EQ(projected.dark_strike_base, Stat{9})
          << "slot " << i << " (Calcified)";
      EXPECT_EQ(projected.ritual_amount, Stat{2})
          << "slot " << i << " (Calcified)";
    } else if (wire.name == "DampCultist") {
      EXPECT_EQ(projected.dark_strike_base, Stat{1})
          << "slot " << i << " (Damp)";
      EXPECT_EQ(projected.ritual_amount, Stat{5})
          << "slot " << i << " (Damp)";
    } else {
