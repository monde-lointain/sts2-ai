...
// ---------------------------------------------------------------------------
// Wave-22-fix-4/H.beta: LRU eviction tests (Q2-ADR-013 Amendment 4
// §LRU-eviction). Tiny-cap synthetic exercises the eviction mechanism
// without 14 GB. Dual-run uses a small deterministic combat (sub-second)
// rather than the full cultist solve so the test runs in normal CI; the
// FP-determinism + identical-eviction-count assertions catch any drift
// from the LRU splice/eviction path.
// ---------------------------------------------------------------------------

// LRU eviction fires deterministically at cap. Inserts 11 distinct synthetic
// keys with cap=10 + walks the splice/eviction path directly.
//   - Insert keys 0..9 → fills the TT exactly to cap (no eviction).
//   - Insert key 10 → evicts the LRU front (key 0) and admits key 10.
//   - peek_score_by_key_for_testing(0) → nullopt (evicted).
//   - peek_score_by_key_for_testing(10) → present.
//   - eviction_count() == 1.
TEST(Search, LRU_EvictionFiresAtCap) {
  using sts2::ai::ZobristKey;

  Search search;
  search.set_tt_cap_for_testing(10);

  auto make_key = [](uint64_t i) -> ZobristKey {
    return ZobristKey{.lo = 0x1000ULL + i, .hi = 0x2000ULL + i};
  };

  // Fill to cap with 10 distinct keys.
  for (uint64_t i = 0; i < 10; ++i) {
    search.tt_insert_for_testing(
        make_key(i),
        Score{.expected_hp = static_cast<double>(i), .expected_rounds = 0.0});
  }
  EXPECT_EQ(search.tt_size(), 10U);
  EXPECT_EQ(search.eviction_count(), 0U);

  // 11th insert evicts key 0 (LRU front; oldest in insertion order).
  search.tt_insert_for_testing(
      make_key(10), Score{.expected_hp = 10.0, .expected_rounds = 0.0});

  EXPECT_EQ(search.tt_size(), 10U);
  EXPECT_EQ(search.eviction_count(), 1U);

  // Key 0 evicted; key 10 present.
  EXPECT_FALSE(search.peek_score_by_key_for_testing(make_key(0)).has_value());
  const auto k10 = search.peek_score_by_key_for_testing(make_key(10));
  ASSERT_TRUE(k10.has_value());
  EXPECT_DOUBLE_EQ(k10->expected_hp, 10.0);

  // Keys 1..9 still present (oldest survivors after eviction).
  for (uint64_t i = 1; i < 10; ++i) {
    EXPECT_TRUE(search.peek_score_by_key_for_testing(make_key(i)).has_value())
        << "key " << i << " unexpectedly evicted";
  }
}

// Dual-run determinism: solving the same state twice yields bit-identical
// Score (FP determinism preserved across the LRU refactor) and identical
// eviction_count. The DefensivePlayPreservesHp setup populates a small TT
// (chance-node enumeration + multi-turn recursion); plenty for catching
// LRU-order non-determinism without the cultist solve's cost.
TEST(Search, LRU_DeterminismDualRun) {
  CardCounts hand;
  hand[CardId::kDefend] = 1;
  CardCounts draw;
  draw[CardId::kStrike] = 1;
  ...
  Search search_a;
  const SearchResult result_a = search_a.solve(build_state());
  const std::size_t evict_a = search_a.eviction_count();

  Search search_b;
  const SearchResult result_b = search_b.solve(build_state());
  const std::size_t evict_b = search_b.eviction_count();

  // FP determinism: bit-identical scores across runs.
  EXPECT_EQ(result_a.status, sts2::ai::SolveStatus::kConverged);
  EXPECT_EQ(result_b.status, sts2::ai::SolveStatus::kConverged);
  EXPECT_DOUBLE_EQ(result_a.score.expected_hp, result_b.score.expected_hp);
  EXPECT_DOUBLE_EQ(result_a.score.expected_rounds,
                   result_b.score.expected_rounds);

  // LRU determinism: identical eviction trajectories. For a small state
  // space well under kMaxTtEntries, eviction_count is 0 on both runs;
  // determinism still requires equality.
  EXPECT_EQ(evict_a, evict_b);
}
